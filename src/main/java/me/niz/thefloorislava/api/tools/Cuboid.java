...
public class Cuboid {
    ...
    public void nextLayer() {
        if (this.isLastLayer())
            throw new IllegalStateException("Selected layer is the last layer, cannot get the next one.");
    ...
    public void previousLayer() {
        if (this.isFirstLayer())
            throw new IllegalStateException("Selected layer is the first layer, cannot get the next one.");

        this.selectedLayer -= this.selectedLayer;
    }

    public boolean isFirstLayer() {
        return this.selectedLayer == 0;
    }

    public boolean isLastLayer() {
        return this.selectedLayer == this.layerList.size() - 1;
    }

    public Layer getSelectedLayer() {
        return this.layerList.get(this.selectedLayer);
    }
    ...
    private void addAllLayers() {
        ...
        for (int i = this.minY; i <= this.maxY; i++) {
            ...
            Layer layer = new Layer(this.minX, this.maxX, i, this.minZ, this.maxZ, this.world);
            layers.add(layer);
